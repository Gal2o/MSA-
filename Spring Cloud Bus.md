...
------
  - ### Spring Cloud Bus를 통해 토큰 값 Refresh 해보기
    ...
    ```
    - #### 4️⃣ rabbitMQ를 통해 갱신 정보를 브로드캐스팅 하기 때문에 한 번의 refresh로 spring-cloud-gateway 서비스에서도 갱신이 된 것을 알 수 있다.
    ```
      com.netflix.discovery.DiscoveryClient    : DiscoveryClient_GATEWAY-SERVICE/Hunmin:gateway-service:8000 - registration status: 204
    ```
  --------
